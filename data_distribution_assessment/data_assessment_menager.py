...
from data_distribution_assessment.distribution_evaluator import evaluator
...
def define_sensitive_attributes_by_task(task):
    if (task == 'Amazon-Google'):
        return ['left_manufacturer', 'right_manufacturer']
    elif (task == 'Walmart-Amazon'):
        return ['left_category', 'right_category']
    elif (task == 'DBLP-GoogleScholar'):
        return ['left_venue', 'right_venue']
    else:
        return []


def get_groups(lines, task):
    sensitive_attributes = define_sensitive_attributes_by_task(task)
    ...
    return evaluator.dynamic_process_chunk(convert_to_dataframe(lines), sensitive_attributes[0], groups)
...
def define_group_index(data_dict, tuple, task):
    keys = list(data_dict.keys())
    column_key = define_sensitive_attributes_by_task(task)[0]
    attribute_value = tuple.__getattribute__(column_key)


    for index, key in enumerate(keys[:-1], start=1):
        if key == attribute_value.strip():
            return index

    return 0 #represent 'others'
